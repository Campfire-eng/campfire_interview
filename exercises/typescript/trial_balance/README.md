...
- A **trial balance** is a point-in-time snapshot: for every account, the net of all lines
  ...
  upstream is broken and every downstream report is suspect.
...
The distinction between "as of a date" and "between two dates" is the crux of the
exercise. They read similarly in English and produce very different numbers.
...
Note that each account produces a single row, and you choose which column its net lands
in based on the sign of the net — an account is not both a debit and a credit at once.
...
inferred from the sign of whatever happens to be in the data. We are also watching for
whether you ask which date field matters before you start — real systems carry both an
effective date and a posted-at timestamp, and they answer different questions.
...
Integer cents only. Balance sheet accounts must never leak into the P&L. This exercise is
