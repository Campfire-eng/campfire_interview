...
Two self-contained exercises. Each folder holds a starter module, a test file that acts as
the specification, and a README describing the accounting problem and the contract you are
implementing.
...
integer cents in both exercises, and dates are `YYYY-MM-DD` strings that compare correctly
with `<` and `>`, so no date library is needed. If something in the spec is genuinely
ambiguous, ask; deciding what to clarify is part of the exercise rather than a distraction
from it.
