...
twenty. We are publishing the whole set on purpose: none of them have a trick, none of
them reward having seen the question before, and we would rather you spend your
preparation time reading about deferred revenue than guessing what we might ask.

Everything here lives in the same domain — an AI-native ERP with a general ledger,
...
concept in the prompt itself, and in the live sessions we will happily explain more. What
we are looking for is whether you take the domain seriously: a ledger has invariants, and
software that breaks them is not "mostly working," it is wrong in a way that shows up in
someone's audit.

## How a session runs

We read the problem, then you drive. Ask us questions — what we are asking for is usually
underspecified in the same way a real ticket is, and noticing that is part of the work.
Think out loud. If you go down a path and change your mind, say so; watching you correct
course tells us more than a clean first draft.

The follow-ups listed under each question are the directions the conversation tends to
go. They are not a checklist you need to exhaust, and running out of time on them is
normal and expected.
...
**On the AI-off sessions.** These are not a statement that we dislike AI tooling; we build
an AI product and we use these tools daily. They exist because we need to see that you
personally hold the domain model — that you know why a journal must balance and what a
locked period protects — since that is the judgment you will be applying when you review
what a model wrote.
...
**Why we ask it.** This is the single invariant the entire product rests on. We want to see
you encode it as a hard failure rather than a comment or a caller's responsibility.

**What a strong answer covers.** Sum of debits equals sum of credits; empty journals
rejected; a line that is somehow both a debit and a credit rejected; all-zero lines
rejected; clear error types that tell a caller what was wrong. Beyond that: an idempotency
key so the same event cannot post twice, a check that the period is open, a check that the
account exists and is postable rather than a header used for grouping, and posted journals
treated as immutable — you reverse, you do not edit. If you get that far, we will talk
about money as integer cents throughout, your rounding policy, how multi-currency lines
would land later, emitting an event after commit without losing it, and what happens when
two posters race the same idempotency key.
...
**Where these go wrong.** Floats for money. Silently inserting a plug line to make an
unbalanced journal fit. Updating posted lines in place. Assuming the UI will validate it.
...
**Why we ask it.** Normal balances and statement construction are the layer Ember will
confidently misreport if the engine underneath is wrong.

**What a strong answer covers.** Netting each account correctly, producing balanced trial
balance columns, and computing the P&L as revenue minus expense over a window. The real
content is the distinction between an as-of date and a period window, net income rolling
to equity, contra accounts, and handling closing entries deliberately rather than by
...
**Where these go wrong.** Treating every positive amount as a debit. Letting balance sheet
accounts leak into the P&L. Conflating `posted_at` and `effective_date` without asking
...
**Why we ask it.** A ledger is append-only financial facts sitting next to mutable
dimensions, and the schema either respects that split or fights it forever.
...
defend; period on the journal. Then: `entity_id` on every fact, period status
...
**Where these go wrong.** A mutable `amount` on lines. No period concept at all. Account
names as free text on lines. Reaching for a document store because the ledger "is just
events."
...
**Why we ask it.** Consolidation is where people quietly create a second source of truth,
and where elimination and ownership either get modelled or get hand-waved.
...
**Where these go wrong.** "Sum all the entities, done." Eliminating by deleting
...
customers, invoices, payments, credit notes, products — with a mapping experience a
controller can actually operate.

**Why we ask it.** Idempotency at the accounting boundary, and then whether sync is a
product surface or a cron job that usually works.
...
three-year backfill that does not crush the GL, a reconciliation job diffing Stripe's list
API against the inbox, contract tests against fixtures, and what the AI layer must never
do — such as posting against a charge id it invented.
...
**Where these go wrong.** Posting to the GL with no unique key. Returning 500 forever.
...
and it is a genuinely hard interface problem.
...
**Where these go wrong.** A chat sidebar as the review tool. No record of who accepted
...
counterparty. This is not a string-distance puzzle.

**Why we ask it.** Matching is constrained assignment with an audit requirement attached —
the explanation is part of the deliverable, not decoration.
...
differences as a tolerance *policy* rather than a magic constant, partial matches that
...
**Where these go wrong.** Matching on vendor name alone. Allowing one GL line into two
...
**Why we ask it.** Deferred revenue has to be a first-class balance, not a spreadsheet
column that someone maintains by hand.

**What a strong answer covers.** Contract, invoice, a deferred revenue liability, and
monthly recognition journals. The real content is performance obligations (licence,
...
**Where these go wrong.** Recognizing cash as revenue. A `revenue_per_month` float on the
...
explicit translation, never a single number.
...
**Where these go wrong.** One `amount_usd` column and nothing else. Revaluing equity the
same way as cash.
...
**Why we ask it.** Close is a control system, not a boolean flag on a row.
...
is allowed to post during close — under which policy, stamped with which run id.

**Follow-ups.** "Two accountants post at the exact lock instant." "A late AP invoice
belongs in a locked month."

**Where these go wrong.** Deleting and rebuilding the month. Enforcing the lock only in
the UI.
...
**Why we ask it.** The difficulty is definitions, grain, and explainability. It is not a
Spark trivia question.
...
over millions of lines, aborting the run when the trial balance does not balance, and
grounding commentary in the top contributing vendors and journals.

**Follow-ups.** "The period isn't closed — do we still publish flux?" "Account 6100 was
split into 6100 and 6101 this year."

**Where these go wrong.** Percent change as the only signal. Ad-hoc recomputation with no
run id. Ignoring account-type direction, so a revenue increase reads as a red flag.
...
**Why we ask it.** A reconciliation is a document with a tie-out and an approver, not a
styled table.

**What a strong answer covers.** Fetching a reconciliation, listing items, certifying it.
Then: preparer and reviewer as genuinely distinct people, balances frozen at
...
**Where these go wrong.** Certifying without snapshotting the balance. Variance as a
free-text comment.
...
**Why we ask it.** Grounding and permission-awareness are the whole difference between this
product and pasting a trial balance into a chatbot.
...
the tool layer rather than the prompt; an honest "I don't know" path when tools disagree;
and prompt injection arriving through invoice memos and PDF attachments. The strongest
version separates deterministic financial functions from generated commentary, insists on
citation completeness for every dollar figure, scopes by period and entity, weighs caching
against freshness during close, refuses to let the model post a journal without dual
...
**Where these go wrong.** The LLM computing the $400k itself. One shared vector store
...
**Why we ask it.** Authorization has to work at data grain, and workflow is part of the
ledger lifecycle rather than something bolted on afterwards.
...
reject-with-reason, an immutable snapshot of what the approver actually saw, and no
...
PTO." "The bill matches the PO — skip the human?"

**Where these go wrong.** An `isAdmin` boolean. Filtering only in the client. A service
...
**Why we ask it.** This is a question about identity and provenance, not about fuzzy string
matching.
...
**Where these go wrong.** A global unique constraint on (amount, date). Silently dropping
one feed.
...
**Why we ask it.** Schedules versus posted reality, stub periods, and never floating money.
...
**Where these go wrong.** Dividing dollars as floats. Amortizing into a locked period by
...
to. Accuracy is the wrong headline metric when one wrong $500k line matters more than a
hundred correct $12 ones.
...
selection as a trade between reviewer hours and misstatement risk, insisting on a held-out
...
**Follow-ups.** "Accuracy is 97% and the controller is furious — what happened?" "We only
...
**Where these go wrong.** A single accuracy number. Evaluating on training data. No notion
...
**Why we ask it.** The interesting question is how control is preserved as automation
...
**Where these go wrong.** Fully dark auto-posting. No unmatched aging metric. A close
checklist that ignores what the automation did.
...
and ships safely — across customers with different charts of accounts, without leaking one
tenant's data into another's model.

**Why we ask it.** Production ML thinking on top of a system of record: data contracts,
tenancy, and safe rollout. Framework names are not the point.
...
**Where these go wrong.** Training on auto-approved predictions with no human labels and no
...
**Why we ask it.** Bi-temporality and evidence — ERP-grade history rather than startup
analytics.
...
**Where these go wrong.** Mutating journal lines and relying on `updated_at`. No stored
